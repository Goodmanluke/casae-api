...
comps_data = [
    ...
    {"id": 3, "address": "789 Pine Rd", "price": 280000, "beds": 3, "baths": 2, "sqft": 1400, "year_built": 1990, "lot_size": 5500},
    {"id": 4, "address": "101 Cedar Blvd", "price": 310000, "beds": 3, "baths": 2, "sqft": 1600, "year_built": 1998, "lot_size": 6200},
    {"id": 5, "address": "202 Maple St", "price": 295000, "beds": 2, "baths": 1, "sqft": 1200, "year_built": 1985, "lot_size": 5000},
]
...
def compute_similarity(comp: dict, price: Optional[float], beds: Optional[int], baths: Optional[int],
                       sqft: Optional[int], year_built: Optional[int], lot_size: Optional[int]) -> float:
    """
...
async def comps_suggest(
    ...
    """
    # Compute scores for all comps
    comps_with_scores = [
        (comp, compute_similarity(comp, price, beds, baths, sqft, year_built, lot_size))
        for comp in comps_data
    ]
    # Sort comps by descending score
    sorted_comps = sorted(comps_with_scores, key=lambda x: x[1], reverse=True)
    # Take top n comps and return only the comp data (not the score)
    top_comps = [comp for comp, score_val in sorted_comps[: n]]
